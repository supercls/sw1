...
function CreateHourLines(data) {
  ...
  for (var i = 0; i < 12; i++) {
    ...
    html1 += `<li style="position: absolute; width: 20px;height: 10px; left: 50%;top: 50%;transform-origin: left center;transform: rotate(${val}deg) translate(
      ${key1}px,${key2}px)"><span style="transform-origin: left center;display:inline-block;transform: rotate(${val}deg) translate(${key1}px,${key2}px)">${val}</span></li>`;
    html +=
